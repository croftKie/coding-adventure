...
export const animator = (imgRef, instructionInputs, height, width) => {
  const pos = imgRef.current.style.transform
    .slice(
      imgRef.current.style.transform.indexOf("(") + 1,
      imgRef.current.style.transform.indexOf(")")
    )
    .replace("px,", "")
    .replace("px", "")
    .split(" ");
      ...
      switch (instructionInputs[i].type) {
        case "forward":
          if (Y - instructionInputs[i].value < 0) {
            ...
            tl.to(imgRef.current, {
              y: Y - Math.abs(instructionInputs[i].value),
            });
        ...
        case "right":
          if (X + instructionInputs[i].value > width) {
            ...
            tl.to(imgRef.current, {
              x: X + Math.abs(instructionInputs[i].value),
            });
...
export const WinCondition = (char, charLocation, goalLocation) => {
  ...
  if (
    charLocation.x <= goalLocation.x + loc.width &&
